...
# used to validate user input on focusOut - more specifically only called to validate tool name in host/port/terminal commands tabs
    ...
    def paintEvent(self, event):
        ...
        for index in range(self.count()):
            ...
            painter.drawText(tabRect, Qt.AlignVCenter |
                             Qt.TextDontClip, self.tabText(index))
        painter.end()
...
# dialog shown when the user selects settings menu
class AddSettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.previousTab = self.settingsTabWidget.tabText(
            self.settingsTabWidget.currentIndex())
        ...
        self.defaultServicesList = [
            "mysql-default", "mssql-default", "ftp-default", "postgres-default", "oracle-default"]

    def setupConnections(self):
        self.browseUsersListButton.clicked.connect(
            lambda: self.wordlistDialog())
        self.browsePasswordsListButton.clicked.connect(
            lambda: self.wordlistDialog('Choose password path'))
        ...
        self.removeToolForTerminalButton.clicked.connect(
            self.removeToolForTerminal)

        self.addServicesButton.clicked.connect(lambda: self.moveService(
            self.servicesAllTableWidget, self.servicesActiveTableWidget))
        self.removeServicesButton.clicked.connect(lambda: self.moveService(
            self.servicesActiveTableWidget, self.servicesAllTableWidget))
        self.addTerminalServiceButton.clicked.connect(lambda: self.moveService(
            self.terminalServicesAllTable, self.terminalServicesActiveTable))
        self.removeTerminalServiceButton.clicked.connect(lambda: self.moveService(
            self.terminalServicesActiveTable, self.terminalServicesAllTable))

        self.toolForHostsTableWidget.clicked.connect(
            self.updateToolForHostInformation)
        self.toolForServiceTableWidget.clicked.connect(
            self.updateToolForServiceInformation)
        self.toolForTerminalTableWidget.clicked.connect(
            self.updateToolForTerminalInformation)

        self.hostActionNameText.textChanged.connect(lambda: self.realTimeToolNameUpdate(
            self.toolForHostsTableWidget, self.hostActionNameText.text()))
        self.portActionNameText.textChanged.connect(lambda: self.realTimeToolNameUpdate(
            self.toolForServiceTableWidget, self.portActionNameText.text()))
        ...
        self.enableAutoAttacks.clicked.connect(
            lambda: self.enableAutoToolsTab())
    ...
    # called by the controller once the config file has been read at start time and also when the cancel button is pressed to forget any changes.
    def setSettings(self, settings):
        self.settings = settings
        # clear any changes the user may have made and canceled.
        self.resetGui()
        # populate the GUI with the new settings
        self.populateSettings()

        # TODO: this is most likely not the best way to do it. we should check if New_Action_1 exists and if so increase the number until it doesn't exist. no need for a self.variable - can be a local one.
    ...
    def updateSettings(self):
        ...
        self.settings.general_default_terminal = str(
            self.terminalComboBox.currentText())
        self.settings.general_max_fast_processes = str(
            self.fastProcessesComboBox.currentText())
        self.settings.general_screenshooter_timeout = str(
            self.screenshotTextinput.text())
        self.settings.general_web_services = str(
            self.webServicesTextinput.text())
        ...
        self.settings.brute_username_wordlist_path = str(
            self.userlistPath.text())
        self.settings.brute_password_wordlist_path = str(
            self.passwordlistPath.text())
    ...
    # called when the cancel button is clicked, to initialise everything
    def resetGui(self):
    ...
    # called by setSettings at start up or when showing the settings dialog after a cancel action. it populates the GUI with the controller's settings object.
    def populateSettings(self):
        # LEO: split it in functions so that it's less confusing and easier to refactor later
    ...
    def populateGeneralTab(self):
        ...
        self.fastProcessesComboBox.setCurrentIndex(
            int(self.settings.general_max_fast_processes) - 1)
        self.screenshotTextinput.setText(
            str(self.settings.general_screenshooter_timeout))
        self.webServicesTextinput.setText(
            str(self.settings.general_web_services))
    ...
    def populateBruteTab(self):
        ...
        self.passwordlistPath.setText(
            self.settings.brute_password_wordlist_path)
...
#       self.cutycaptPathInput.setText(self.settings.tools_path_cutycapt)
        ...
        self.toolForHostsTableWidget.setRowCount(
            len(self.settings.hostActions))
        for row in range(len(self.settings.hostActions)):
            ...
            self.toolForHostsTableWidget.item(row, 0).setText(
                self.settings.hostActions[row][1])

        self.toolForServiceTableWidget.setRowCount(
            len(self.settings.portActions))
        for row in range(len(self.settings.portActions)):
            self.toolForServiceTableWidget.setItem(row, 0, QTableWidgetItem())
            self.toolForServiceTableWidget.item(row, 0).setText(
                self.settings.portActions[row][1])
        ...
        for row in range(len(self.settings.portActions)):
            ...
            self.servicesAllTableWidget.item(row, 0).setText(
                self.settings.portActions[row][3])

        self.toolForTerminalTableWidget.setRowCount(
            len(self.settings.portTerminalActions))
        for row in range(len(self.settings.portTerminalActions)):
            ...
            self.toolForTerminalTableWidget.item(row, 0).setText(
                self.settings.portTerminalActions[row][1])
        self.terminalServicesAllTable.setRowCount(
            len(self.settings.portTerminalActions))
        for row in range(len(self.settings.portTerminalActions)):
            self.terminalServicesAllTable.setItem(row, 0, QTableWidgetItem())
            self.terminalServicesAllTable.item(row, 0).setText(
                self.settings.portTerminalActions[row][3])

    # TODO: this one is still to big and ugly. needs work.
    def populateAutomatedAttacksTab(self):
        self.typeDic = {}
        for i in range(len(self.settings.portActions)):
            # the dictionary contains the name, the text input and the layout for each tool
            self.typeDic.update({self.settings.portActions[i][1]: [
        ...
        for keyNum in range(len(self.settings.portActions)):
            ...
            if self.settings.portActions[keyNum][1] not in self.defaultServicesList:
                ...
                self.typeDic[self.settings.portActions[keyNum][1]][0].setText(
                    self.settings.portActions[keyNum][1])
                self.typeDic[self.settings.portActions[keyNum]
                             [1]][0].setFixedWidth(150)
                ...
                if not foundToolInAA:
                    self.typeDic[self.settings.portActions[keyNum][1]][1].setText(
                        self.settings.portActions[keyNum][3])

                self.typeDic[self.settings.portActions[keyNum]
                             [1]][1].setFixedWidth(300)
                ...
                self.typeDic[self.settings.portActions[keyNum]
                             [1]][3].addItem(self.enabledSpacer)
                self.typeDic[self.settings.portActions[keyNum][1]][3].addWidget(
                    self.typeDic[self.settings.portActions[keyNum][1]][2])
                self.scrollVerLayout.addLayout(
                    self.typeDic[self.settings.portActions[keyNum][1]][3])

            # populate the automated attacks tools tab with every tool that IS a default creds check
            else:
                ...
                self.defaultBoxVerlayout.addLayout(
                    self.typeDic[self.settings.portActions[keyNum][1]][3])
    ...
    def switchTabClick(self):
        if self.settingsTabWidget.tabText(self.settingsTabWidget.currentIndex()) == 'Tools':
            self.previousToolTab = self.ToolSettingsTab.tabText(
                self.ToolSettingsTab.currentIndex())
...
#               self.updateTerminalActions()                            # LEO: commented out because it didn't look right, please check!
    ...
    # called by validation functions to display (or not) a red border around a text input widget when input is (in)valid. easier to change stylesheets in one place only.
    ...
    def validateString(self, widget):
        # TODO: this is too strict in some cases...
        if not validateString(str(widget.text())):
            self.toggleRedBorder(widget, True)
            return False
        else:
            self.toggleRedBorder(widget, False)
    ...
    def validateCurrentTab(self, tab):
        ...
        if tab == 'General':
            ...
            self.ToolSettingsTab.setCurrentIndex(0)
            currentToolsTab = self.ToolSettingsTab.tabText(
                self.ToolSettingsTab.currentIndex())
            if currentToolsTab == 'Tool Paths':
                ...
                print('>>>> we should never be here. potential bug. 1')
            ...
            # LEO: added this just to help when testing. we'll remove it later.
            print('>>>> we should never be here. potential bug. 2')
    ...
    def validateGeneralTab(self):
        ...
        # TODO: this is too strict! no spaces or comma allowed? we can clean up for the user in some simple cases. actually, i'm not sure we even need to split.
    ...
    # LEO: do NOT change the order of the AND statements otherwise validation may not take place if first condition is False
    def validateBruteTab(self):
        validationPassed = self.validatePath(self.userlistPath)
        validationPassed = self.validatePath(
            self.passwordlistPath) and validationPassed
        validationPassed = self.validateString(
            self.defaultUserText) and validationPassed
        validationPassed = self.validateString(
            self.defaultPassText) and validationPassed
        return validationPassed

    def toolPathsValidate(self):
        validationPassed = self.validateFile(self.nmapPathInput)
        validationPassed = self.validateFile(
            self.hydraPathInput) and validationPassed
#       validationPassed = self.validateFile(self.cutycaptPathInput) and validationPassed
        validationPassed = self.validateFile(
            self.textEditorPathInput) and validationPassed
        return validationPassed
...
#   def commandTabsValidate(self):                                      # LEO: renamed and refactored
    ...
    def validateCommandTabs(self, nameInput, labelInput, commandInput):
        ...
        # the self.validationPassed comes from the focus out event
        if self.validationPassed == False:
            # TODO: this seems like a dodgy way to do it - functions should not depend on hope :) . maybe it's better to simply validate again. code will be clearer too.
            self.toggleRedBorder(nameInput, True)
            validationPassed = False
        else:
            self.toggleRedBorder(nameInput, False)

        validationPassed = self.validateStringWithSpace(
            labelInput) and validationPassed
        validationPassed = self.validateCommandFormat(
            commandInput) and validationPassed
        return validationPassed
...
#   def validateInput(self):                                            # LEO: renamed
    ...
    def validateToolName(self):
        ...
        if tmplineEdit:
            ...
            # LEO: when the first condition is True the validateUniqueToolName is never called (bad if we want to show a nice error message for the unique key)
            if row != -1:
    ...
    # LEO: the function that calls this one already knows the selectGroup stuff so no need to duplicate.
    ...
    # LEO: renamed and fixed bugs. TODO: this function is being called way too often. something seems wrong in the overall logic
    def validateStagedNmapTab(self):
        validationPassed = self.validateNmapPorts(self.stage1Input)
        validationPassed = self.validateNmapPorts(
            self.stage2Input) and validationPassed
        validationPassed = self.validateNmapPorts(
            self.stage3Input) and validationPassed
        validationPassed = self.validateNmapPorts(
            self.stage4Input) and validationPassed
        validationPassed = self.validateNmapPorts(
            self.stage5Input) and validationPassed
        return validationPassed
    ...
    def addToolForHost(self):
        ...
        if self.validateCommandTabs(self.hostActionNameText, self.hostLabelText, self.hostCommandText):
            ...
            self.toolForHostsTableWidget.setItem(
                currentRows, 0, QTableWidgetItem())
            self.toolForHostsTableWidget.item(self.toolForHostsTableWidget.rowCount(
            )-1, 0).setText('New_Action_'+str(self.hostActionsNumber))
            self.toolForHostsTableWidget.selectRow(currentRows)
            self.settings.hostActions.append(
                ['', 'New_Action_'+str(self.hostActionsNumber), ''])
            self.hostActionsNumber += 1
            self.updateToolForHostInformation()
    ...
    def updateHostActions(self):
        self.settings.hostActions[self.hostTableRow][0] = str(
            self.hostLabelText.text())
        self.settings.hostActions[self.hostTableRow][2] = str(
            self.hostCommandText.text())
...
#           self.hostCommandText.setStyleSheet("border: 1px solid grey;")
            ...
            if self.toolForHostsTableWidget.item(self.hostTableRow, 0) is not None:
                key = self.toolForHostsTableWidget.item(
                    self.hostTableRow, 0).text()
                for tool in self.settings.hostActions:
                    if tool[1] == key:
    ...
    # the name still sucks, sorry. at least it's refactored
    ...
    def addToolForService(self):
        ...
        if self.validateCommandTabs(self.portActionNameText, self.portLabelText, self.portCommandText):
            ...
            self.toolForServiceTableWidget.setItem(
                currentRows, 0, QTableWidgetItem())
            self.toolForServiceTableWidget.item(self.toolForServiceTableWidget.rowCount(
            )-1, 0).setText('New_Action_'+str(self.portActionsNumber))
            self.toolForServiceTableWidget.selectRow(currentRows)
            self.settings.portActions.append(
                ['', 'New_Action_'+str(self.portActionsNumber), ''])
            self.portActionsNumber += 1
            self.updateToolForServiceInformation()
    ...
    def updatePortActions(self):
        self.settings.portActions[self.portTableRow][0] = str(
            self.portLabelText.text())
        self.settings.portActions[self.portTableRow][2] = str(
            self.portCommandText.text())
...
#           self.portCommandText.setStyleSheet("border: 1px solid grey;")
            ...
            if self.toolForServiceTableWidget.item(self.portTableRow, 0) is not None:
                key = self.toolForServiceTableWidget.item(
                    self.portTableRow, 0).text()
                for tool in self.settings.portActions:
                    if tool[1] == key:
                        ...
                        if len(tool) == 4:
                            ...
                            self.terminalServicesActiveTable.setRowCount(
                                len(servicesList))
                            for i in range(len(servicesList)):
                                self.terminalServicesActiveTable.setItem(
                                    i, 0, QTableWidgetItem())
                                self.terminalServicesActiveTable.item(
                                    i, 0).setText(str(servicesList[i]))
        else:
    ...
    def addToolForTerminal(self):
        ...
        if self.validateCommandTabs(self.terminalActionNameText, self.terminalLabelText, self.terminalCommandText):
            ...
            self.toolForTerminalTableWidget.setItem(
                currentRows, 0, QTableWidgetItem())
            self.toolForTerminalTableWidget.item(self.toolForTerminalTableWidget.rowCount(
            )-1, 0).setText('New_Action_'+str(self.terminalActionsNumber))
            self.toolForTerminalTableWidget.selectRow(currentRows)
            self.settings.portTerminalActions.append(
                ['', 'New_Action_'+str(self.terminalActionsNumber), ''])
            self.terminalActionsNumber += 1
            self.updateToolForTerminalInformation()
    ...
    def updateTerminalActions(self):
        self.settings.portTerminalActions[self.terminalTableRow][0] = str(
            self.terminalLabelText.text())
        self.settings.portTerminalActions[self.terminalTableRow][2] = str(
            self.terminalCommandText.text())
...
#           self.terminalCommandText.setStyleSheet("border: 1px solid grey;")
            ...
            if self.toolForTerminalTableWidget.item(self.terminalTableRow, 0) is not None:
                key = self.toolForTerminalTableWidget.item(
                    self.terminalTableRow, 0).text()
                for tool in self.settings.portTerminalActions:
                    if tool[1] == key:
    ...
    # toggles select/deselect all default creds checkboxes
    def toggleDefaultServices(self):
    ...
    def moveService(self, src, dst):
        if src.selectionModel().selectedRows():
            ...
            dst.item(dst.rowCount() - 1,
                     0).setText(str(src.item(row, 0).text()))
            src.removeRow(row)
    ...
    def setupLayout(self):
        ...
        self.settingsTabWidget.setTabBar(
            SettingsTabBarWidget(width=200, height=25))
        self.settingsTabWidget.setTabPosition(
            QTabWidget.West)    # put the tab titles on the left
        ...
        self.settingsTabWidget.addTab(
            self.AutoAttacksSettingsTab, "Automated Attacks")
    ...
    def setupGeneralTab(self):
        ...
        self.fastProcessesComboBox.setStyleSheet(
    ...
    def setupToolsTab(self):
        ...
        self.ToolSettingsTab.addTab(
            self.portTerminalActionsWidget, "Terminal Commands")
...
#       self.toolsPathVerLayout.addLayout(self.cutycaptPathHorLayout)
        ...
        self.toolsPathHorSpacer = QSpacerItem(
            50, 0)                     # right margin spacer
    ...
    def setupHostCommandsTab(self):
        ...
        self.toolForHostsTableWidget.setSelectionBehavior(
            QAbstractItemView.SelectRows)
        self.toolForHostsTableWidget.setFixedWidth(180)
        # to make the cells of the table read only
        self.toolForHostsTableWidget.setShowGrid(False)
        self.toolForHostsTableWidget.setEditTriggers(
            QAbstractItemView.NoEditTriggers)

        self.toolForHostsTableWidget.setColumnCount(1)
        self.toolForHostsTableWidget.setHorizontalHeaderItem(
            0, QTableWidgetItem())
        ...
        self.toolForHostsTableWidget.verticalHeader(
        ).setVisible(False)  # row header - is hidden
        self.toolForHostsTableWidget.setVerticalHeaderItem(
            0, QTableWidgetItem())
    ...
    def setupPortCommandsTab(self):
        ...
        self.toolForServiceTableWidget.setSelectionBehavior(
            QAbstractItemView.SelectRows)
        self.toolForServiceTableWidget.setFixedWidth(180)
        self.toolForServiceTableWidget.setShowGrid(False)
        self.toolForServiceTableWidget.setEditTriggers(
            QAbstractItemView.NoEditTriggers)
        # table headers
        self.toolForServiceTableWidget.setColumnCount(1)
        self.toolForServiceTableWidget.setHorizontalHeaderItem(
            0, QTableWidgetItem())
        ...
        self.toolForServiceTableWidget.setVerticalHeaderItem(
            0, QTableWidgetItem())
        ...
        self.servicesAllTableWidget.setSelectionBehavior(
            QAbstractItemView.SelectRows)
        ...
        self.servicesAllTableWidget.setHorizontalHeaderItem(
            0, QTableWidgetItem())
        ...
        self.servicesActiveTableWidget.setSelectionBehavior(
            QAbstractItemView.SelectRows)
        ...
        self.servicesActiveTableWidget.setHorizontalHeaderItem(
            0, QTableWidgetItem())
        ...
        # space above and below arrow buttons
        self.spacer4 = QSpacerItem(0, 90)
        ...
        # space left of multiple choice widget
        ...
        # bottom right space
        self.spacer1 = QSpacerItem(0, 50)
        self.verLayout1.addItem(self.spacer1)

        self.globLayoutPortActions = QHBoxLayout(self.PortActionsWidget)
        self.globLayoutPortActions.addLayout(self.verLayoutPortActions)

        # space between left and right layouts
        self.spacer5 = QSpacerItem(10, 0)
        self.globLayoutPortActions.addItem(self.spacer5)
        self.globLayoutPortActions.addLayout(self.verLayout1)
        # right margin space
    ...
    def setupTerminalCommandsTab(self):
        ...
        self.toolForTerminalTableWidget = QTableWidget(
            self.portTerminalActionsWidget)
        self.toolForTerminalTableWidget.setSelectionBehavior(
            QAbstractItemView.SelectRows)
        self.toolForTerminalTableWidget.setFixedWidth(180)
        self.toolForTerminalTableWidget.setShowGrid(False)
        # to make the cells of the table read only
        self.toolForTerminalTableWidget.setEditTriggers(
            QAbstractItemView.NoEditTriggers)
        # table headers
        self.toolForTerminalTableWidget.setColumnCount(1)
        self.toolForTerminalTableWidget.setHorizontalHeaderItem(
            0, QTableWidgetItem())
        ...
        self.toolForTerminalTableWidget.setVerticalHeaderItem(
            0, QTableWidgetItem())
        ...
        self.terminalServicesAllTable.setSelectionBehavior(
            QAbstractItemView.SelectRows)
        ...
        self.terminalServicesAllTable.setHorizontalHeaderItem(
            0, QTableWidgetItem())
        self.terminalServicesAllTable.horizontalHeaderItem(
            0).setText("Available Services")
        ...
        self.terminalServicesActiveTable.setSelectionBehavior(
            QAbstractItemView.SelectRows)
        ...
        self.terminalServicesActiveTable.setHorizontalHeaderItem(
            0, QTableWidgetItem())
        self.terminalServicesActiveTable.horizontalHeaderItem(
            0).setText("Applied Services")
        ...
        self.globLayoutTerminalActions = QHBoxLayout(
            self.portTerminalActionsWidget)
    ...
    def setupAutomatedAttacksTab(self):
        ...
        self.AutoAttacksSettingsTab.addTab(
            self.GeneralAutoSettingsWidget, "General")
        self.AutoToolsWidget = QWidget()
        self.AutoAttacksSettingsTab.addTab(
            self.AutoToolsWidget, "Tool Configuration")
    ...
    def wordlistDialog(self, title='Choose username path'):
        if title == 'Choose username path':
            path = QFileDialog.getExistingDirectory(
                self, title, '/',  QFileDialog.ShowDirsOnly | QFileDialog.DontResolveSymlinks)
